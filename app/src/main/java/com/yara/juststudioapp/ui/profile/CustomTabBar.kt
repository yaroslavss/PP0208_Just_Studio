...
import com.yara.juststudioapp.databinding.LayoutTabBarBinding
...
class CustomTabBar(context: Context?, attrs: AttributeSet?) :
    ...
    private fun initTabTv(tabName: String, index: Int) = TextView(context).apply {
        ...
        setTextColor(ContextCompat.getColor(this.context, R.color.teal_200))
        textSize = 12f
        setOnClickListener {
            onTabSelected(index)
            binding.viewIndicator.setTextColor(ContextCompat.getColor(this.context, R.color.red))
        }
    }
